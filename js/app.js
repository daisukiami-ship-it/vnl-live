/**
 * VNL Live - Volleyball Live Center
 * Data proxied via local Python backend from volleyballworld.com
 * Match schedule from VBTV -> VW full mapping
 */

// Auto-detect deployment domain, use 127.0.0.1:8888 for local dev
...
const APP = {
  state: {
    ...
    matchData: null,    // live match data (API)
    playersA: [],       // Team A player stats (API)
    playersB: [],       // Team B player stats (API)
    rosterA: [],        // Team A roster (CSV)
    rosterB: [],        // Team B roster (CSV)
  },
  ...
  switchHlsSource(m3u8Url) {
    ...
    if (!m3u8Url) {
      ...
      placeholder.querySelector('p').textContent = 'No stream source';
      return;
    }
    ...
    if (Hls.isSupported()) {
      ...
      hls.on(Hls.Events.ERROR, (event, data) => {
        if (data.fatal) {
          ...
          placeholder.classList.remove('hidden');
          placeholder.querySelector('p').textContent = 'Stream unavailable';
        }
      });
  ...
  async loadSchedule() {
    try {
      ...
      document.getElementById('schedule-list').innerHTML =
        '<div class="loading-hint">Schedule load failed</div>';
    }
  },
  ...
  renderSchedule() {
    ...
    if (!list.length) {
      container.innerHTML = '<div class="loading-hint">No matches</div>';
      return;
    }
    ...
    const groups = {};
    list.forEach(m => {
      const d = m.date || m.localTime?.slice(0, 10) || 'TBD';
    ...
    const timeStr = (m) => {
      if (m.localTime) return m.localTime.slice(11, 16);
      return '--:--';
    };
    const stateLabel = (m) => {
      switch (m.eventState) {
        case 'LIVE': return '<span class="sch-state live">LIVE</span>';
        case 'PRE_LIVE': return `<span class="sch-state upcoming">${timeStr(m)}</span>`;
        case 'FINISHED': return '<span class="sch-state finished">Finished</span>';
  ...
  selectMatch(match) {
    ...
    // Switch video source
    this.switchHlsSource(match.m3u8 || null);

    // Load rosters
    this.loadRosters(match.codeA, match.codeB);

    // Load match data and player stats
    this.loadMatchData();
    this.loadPlayerStats();

    // Update player stats tab labels
  ...
  updateScoreBar() { /* score bar removed */ },
  ...
  renderMatchBar(data) { /* score bar removed */ },
  ...
  parsePlayerStats(html) {
    ...
    // Replace API English names with CSV Chinese names
    const buildNameMap = (roster) => {
      const map = {};
    ...
    // Filter left-side roster to only show players with actual stats
    this.filterRostersByStats();
  ...
  /* Filter roster by shirt numbers present in player stats */
  ...
  /* ---- Mid column: Match Statistics by Player ---- */
  currentStatTeam: 'a',
  ...
  renderPlayerStatsDetail() {
    ...
    if (!players || players.length === 0) {
      container.innerHTML = '<div class="loading-hint">No data (match not started or unavailable)</div>';
      return;
    }
